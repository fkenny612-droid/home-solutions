...
export default function ProviderEditProfile() {
  ...
  const handleSave = async () => {
    ...
    setSaving(true)
    try {
      await api.auth.updateProfile({ firstName: firstName.trim(), lastName: lastName.trim(), ...(email.trim() ? { email: email.trim() } : {}) })
      ...
      <ScrollView contentContainerStyle={s.body} keyboardShouldPersistTaps="handled">
        ...
        <View style={s.fieldGroup}>
          <View style={s.field}>
            ...
            <TextInput style={s.input} value={firstName} onChangeText={setFirstName} placeholder="First name" placeholderTextColor={colors.gray400} autoCapitalize="words" />
          </View>
          <View style={[s.field, s.fieldBorder]}>
            <Text style={s.label}>Last name</Text>
            <TextInput style={s.input} value={lastName} onChangeText={setLastName} placeholder="Last name" placeholderTextColor={colors.gray400} autoCapitalize="words" />
          </View>
          <View style={[s.field, s.fieldBorder]}>
            <Text style={s.label}>Email</Text>
            <TextInput style={s.input} value={email} onChangeText={setEmail} placeholder="your@email.com" placeholderTextColor={colors.gray400} keyboardType="email-address" autoCapitalize="none" />
          </View>
        </View>
